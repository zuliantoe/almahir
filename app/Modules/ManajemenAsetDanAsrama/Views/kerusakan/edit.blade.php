...
<div class="container-fluid">
    ...
    <div class="row justify-content-center">
        <div class="col-md-8">
            <x-card title="Edit Kerusakan Aset" icon="fas fa-edit">
                <form action="{{ route('manajemenasetdanasrama.kerusakan.update', $kerusakan->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('aset_id') is-invalid @enderror" id="aset_id" name="aset_id" required>
                                    ...
                                    @foreach($aset as $a)
                                        <option value="{{ $a->id }}" data-tanggal="{{ $a->tanggal_pengadaan ? \Carbon\Carbon::parse($a->tanggal_pengadaan)->format('Y-m-d') : '' }}" {{ old('aset_id', $kerusakan->aset_id) == $a->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" class="form-control @error('tanggal_kerusakan') is-invalid @enderror" id="tanggal_kerusakan" name="tanggal_kerusakan" value="{{ old('tanggal_kerusakan', $kerusakan->tanggal_kerusakan ? $kerusakan->tanggal_kerusakan->format('Y-m-d') : ($kerusakan->tanggal_rusak ? $kerusakan->tanggal_rusak->format('Y-m-d') : date('Y-m-d'))) }}" required>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('status_penanganan') is-invalid @enderror" id="status_penanganan" name="status_penanganan" required>
                                    <option value="belum_ditangani" {{ old('status_penanganan', $kerusakan->status_penanganan) == 'belum_ditangani' ? 'selected' : '' }}>Belum Ditangani</option>
                                    <option value="sedang_ditangani" {{ old('status_penanganan', $kerusakan->status_penanganan) == 'sedang_ditangani' ? 'selected' : '' }}>Sedang Ditangani</option>
                                    <option value="selesai" {{ old('status_penanganan', $kerusakan->status_penanganan) == 'selesai' ? 'selected' : '' }}>Selesai</option>
                                </select>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('deskripsi_kerusakan') is-invalid @enderror" id="deskripsi_kerusakan" name="deskripsi_kerusakan" rows="4" required>{{ old('deskripsi_kerusakan', $kerusakan->deskripsi_kerusakan) }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('catatan') is-invalid @enderror" id="catatan" name="catatan" rows="3">{{ old('catatan', $kerusakan->catatan) }}</textarea>
                        @error('catatan')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
                    </div>
