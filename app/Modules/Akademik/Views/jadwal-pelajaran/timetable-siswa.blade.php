    ...
    <div class="row mb-3">
        <div class="col-12">
            ...
            @if($rombelSiswa && $rombelSiswa->rombel)
                <p class="text-muted">Kelas: <strong>{{ $rombelSiswa->rombel->nama_rombel }}</strong> ({{ optional($rombelSiswa->rombel->tahunAjaran)->tahunajaran }})</p>
            @endif
        </div>
    </div>
    ...
    @if(!$rombelSiswa)
        <x-card type="warning" outline>
            <div class="text-center py-5">
                ...
                <h5>Anda Belum Ditambahkan ke Kelas</h5>
                        ...
                        <ul class="list-group list-group-flush">
                            @foreach($jadwalHari->sortBy('jamke') as $j)
                                <li class="list-group-item p-2 d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="badge badge-pill badge-light">{{ optional($j->guru)->nama ?? '-' }}</span>
                                </li>
                            @endforeach
                        </ul>
                    </x-card>
