...
<div class="container-fluid">
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ route('keuangan.tujuans.index') }}" class="btn btn-sm btn-secondary shadow-sm">
        ...
        <div class="col-lg-6">
            <div class="card shadow-sm border-0 mb-4 rounded-xl overflow-hidden">
                <div class="card-header py-3 bg-white border-bottom border-light">
                    <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-edit mr-2"></i> Edit Data Kategori</h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('keuangan.tujuans.update', $tujuan->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-group mb-4">
                            <label for="nama" class="small font-weight-bold text-muted mb-2">Nama Kategori Pengeluaran</label>
                            <input type="text" 
                                   class="form-control form-control-user bg-light border-0 shadow-sm rounded-lg @error('nama') is-invalid @enderror" 
                                   id="nama" 
                                   name="nama" 
                                   placeholder="Contoh: Gaji, Listrik, Konsumsi, dll" 
                                   value="{{ old('nama', $tujuan->nama) }}" 
                                   required 
                                   autofocus>
                            @error('nama')
                        ...
                        <div class="d-flex justify-content-end mt-4">
                            <button type="button" class="btn btn-light mr-3 px-4" onclick="window.history.back()">Batal</button>
