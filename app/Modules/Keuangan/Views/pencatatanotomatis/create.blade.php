...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="h3 mb-0 text-gray-800">Tambah Pencatatan Otomatis</h1>
    ...
    <div class="card shadow-sm border-0 mb-4 rounded-xl overflow-hidden">
        <div class="card-header py-3 bg-white border-bottom border-light">
            <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-plus-circle mr-2"></i> Form Tambah Pencatatan</h6>
        </div>
        <div class="card-body">
            <form id="otomatisForm" action="{{ route('keuangan.pencatatanotomatis.store') }}" method="POST">
                @csrf
                <div class="row">
                    ...
                    <div class="col-md-6 mb-4" id="sumber_wrapper" style="display: none;">
                        ...
                        <select name="sumber_id" id="sumber_id" class="form-control bg-light border-0 shadow-sm rounded-lg @error('sumber_id') is-invalid @enderror">
                            ...
                            @foreach($sumbers as $sumber)
                                <option value="{{ $sumber->id }}" {{ old('sumber_id') == $sumber->id ? 'selected' : '' }}>{{ $sumber->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-4" id="tujuan_wrapper" style="display: none;">
                        ...
                        <select name="tujuan_id" id="tujuan_id" class="form-control bg-light border-0 shadow-sm rounded-lg @error('tujuan_id') is-invalid @enderror">
                            ...
                            @foreach($tujuans as $tujuan)
                                <option value="{{ $tujuan->id }}" {{ old('tujuan_id') == $tujuan->id ? 'selected' : '' }}>{{ $tujuan->nama }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <select name="frekuensi" id="frekuensi" class="form-control bg-light border-0 shadow-sm rounded-lg @error('frekuensi') is-invalid @enderror" required>
                            <option value="sekali" {{ old('frekuensi') == 'sekali' ? 'selected' : '' }}>Sekali Saja</option>
                            <option value="harian" {{ old('frekuensi') == 'harian' ? 'selected' : '' }}>Setiap Hari</option>
                            <option value="bulanan" {{ old('frekuensi') == 'bulanan' ? 'selected' : '' }}>Setiap Bulan</option>
                        </select>
                    </div>
                    ...
                    <div class="col-md-6 mb-4">
                        ...
                        <input type="time" 
                               ...
                               value="{{ old('waktu_eksekusi') ?? date('H:i') }}" 
                               required>
                    </div>
                    ...
                    <div class="col-md-12 mb-4">
                        ...
                        <textarea name="deskripsi" 
                                  ...
                                  rows="3"
                                  placeholder="Tambahkan deskripsi pencatatan otomatis">{{ old('deskripsi') }}</textarea>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end mt-4">
                    ...
                    <button type="submit" class="btn btn-primary px-5 shadow-sm rounded-lg">
                        <i class="fas fa-save mr-2"></i> Simpan Pencatatan Otomatis
                    </button>
                </div>
            </form>
        </div>
