...
<div class="container-fluid">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <h1 class="h3 mb-1 text-gray-800 font-weight-bold">Jadwal Pelajaran Mingguan</h1>
            @if($rombelSiswa && $rombelSiswa->rombel)
                <p class="text-muted">
                    <i class="fas fa-graduation-cap mr-1"></i> Kelas: <strong class="text-primary">{{ $rombelSiswa->rombel->nama_rombel }}</strong> 
                    <span class="mx-2">|</span> 
                    <i class="fas fa-calendar-alt mr-1"></i> Tahun Ajaran: <strong>{{ optional($rombelSiswa->rombel->tahunAjaran)->tahunajaran }}</strong>
                </p>
            @endif
        </div>
    </div>
            ...
            <form action="{{ route('akademik.jadwal-pelajaran.index') }}" method="GET" class="row align-items-center">
                <div class="col-lg-4 col-md-5 mb-3 mb-md-0">
                    <div class="d-flex align-items-center">
                        <div class="bg-info-soft p-3 rounded-circle mr-3">
                            <i class="fas fa-calendar-check fa-lg text-info"></i>
                        </div>
                        <div>
                            <div class="text-xs font-weight-bold text-info text-uppercase mb-0">Tahun Ajaran Aktif</div>
                ...
                <div class="col-lg-3 col-md-3">
                    <x-btn type="submit" class="btn-info w-100 rounded-pill px-4 shadow-sm">
    ...
    @if(!$rombelSiswa)
        <x-card type="warning" outline class="border-0 shadow-sm rounded-xl">
            <div class="text-center py-5">
                <i class="fas fa-info-circle fa-3x text-warning mb-3"></i>
                <h5 class="font-weight-bold">Anda Belum Ditambahkan ke Kelas</h5>
                <p class="text-muted">Hubungi guru atau admin untuk pendaftaran kelas.</p>
            </div>
        </x-card>
    @elseif($rawJadwal->isEmpty())
        <x-card type="info" outline class="border-0 shadow-sm rounded-xl">
            <div class="text-center py-5">
                ...
                <h5 class="font-weight-bold">Jadwal Belum Tersedia</h5>
                <p class="text-muted">Jadwal pelajaran untuk kelas Anda belum diatur.</p>
            </div>
        </x-card>
    @else
        ...
        <div class="card shadow-lg border-0 rounded-xl mb-4 overflow-hidden">
            <div class="card-header bg-primary py-3 d-flex justify-content-between align-items-center">
                ...
                <span class="badge badge-light px-3 py-2 text-primary font-weight-bold rounded-pill shadow-sm">{{ $rawJadwal->count() }} Sesi Belajar</span>
            </div>
            <div class="card-body p-3">
                <div class="table-responsive">
                    <table class="table-timetable" id="timetable-siswa">
                        <thead>
                            <tr>
                                                ...
                                                <div class="schedule-card" style="--accent-color: {{ $color['border'] }}; --text-color: {{ $color['text'] }}; background-color: {{ $color['bg'] }}; border: 1px solid {{ $color['border'] }}2b;">
                                                    ...
                                                    <div>
                                                        <div class="schedule-info">
                                                            ...
                                                            <span>{{ optional($j->guru)->nama ?? '-' }}</span>
                                                        </div>
                                                        <div class="schedule-info">
        ...
        <div class="d-flex align-items-center mt-5 mb-4">
            ...
            <h4 class="h4 mb-0 text-gray-800 font-weight-bold">Ringkasan Sesi Belajar Harian</h4>
        </div>
                                    ...
                                    <li class="timeline-item {{ $isToday ? 'active' : '' }}">
                                        ...
                                        <div class="timeline-content" style="border-left-color: {{ $color['border'] }}; background: {{ $color['bg'] }}">
                                            ...
                                            <div class="timeline-subtitle">
                                                <i class="fas fa-user-tie mr-1 opacity-7"></i> {{ optional($j->guru)->nama ?? '-' }}
                                            </div>
                                        </div>
                                    </li>
                                @endforeach
