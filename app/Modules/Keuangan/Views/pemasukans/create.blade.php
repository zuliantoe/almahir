...
<div class="container-fluid">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Tambah Pemasukan Baru</h1>
        ...
        <div class="col-lg-8">
            <div class="card shadow-sm border-0 mb-4 rounded-xl overflow-hidden">
                <div class="card-header py-3 bg-white border-bottom border-light">
                    <h6 class="m-0 font-weight-bold text-success"><i class="fas fa-plus-circle mr-2"></i> Form Tambah Pemasukan</h6>
                </div>
                <div class="card-body">
                    <form id="incomeForm" action="{{ route('keuangan.pemasukans.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <select name="sumber_id" id="sumber_id" class="form-control bg-light border-0 shadow-sm rounded-lg @error('sumber_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($sumbers as $sumber)
                                        <option value="{{ $sumber->id }}" {{ old('sumber_id') == $sumber->id ? 'selected' : '' }}>
                                            {{ $sumber->nama }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-4">
                                ...
                                <textarea name="deskripsi" 
                                          ...
                                          rows="2"
                                          placeholder="Tambahkan deskripsi pemasukan">{{ old('deskripsi') }}</textarea>
                                @error('deskripsi')
                                    <div class="invalid-feedback ml-2">{{ $message }}</div>
                                @enderror
                            </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Format value pada saat inisiasi view (penting jika ada error validasi lain)
